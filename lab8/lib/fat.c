...
void parseDentry(Dentry* dentry,vnode* node,vnode* parent,int display){
	...
	node->internal=malloc(sizeof(Content));
...
unsigned int getChainLen(unsigned int id){
	...
	while(1){
		...
		if(id>=2&&id<=0xFFFFFEF){
			...
			uart_printf("invalid table entry!");
		}
	}
	return ret;
}
...
void syncFAT(file* f){
	...
	if(content->type==1)uart_printf("TODO");
	...
	{//update table
		...
		if(old_blks>cur_blks)uart_printf("old_blks>cur_blks");
		int tail=content->id;
	...
	{//update data
		...
		for(int i=0;i<content->len;i+=512){
			if(cur<2||cur>0xFFFFFEF)uart_printf("wrong table id!");
	...
	{//update parent
		...
		if(pos==-1)uart_printf("sync a unknown file!");
		int dirsize=512/32;
...
void parseMBR(){
	...
	if(buf[446]!=0x80){
        uart_printf("invalid partition status!\n");
        while(1) {}
    }
...
int lookupFAT(vnode* dir_node,vnode** target,const char* component_name){
	...
	if(content->type!=1)uart_printf("not a directory!");
	if(content->cache==0){
		unsigned char* buf;
		unsigned int len=getChain(content->id,&buf);
		vnode** childs=malloc(len/32*8);
		int cnt=0;
		for(int i=0;i<len;i+=32){
			if(buf[i]==0)break;
			childs[cnt]=malloc(sizeof(vnode));
...
void parseFAT32(){
	...
	if(buf[510]!=0x55||buf[511]!=0xaa)uart_printf("invalid FAT signature!");
	unsigned int sector_size = buf[12]; // 0x0B ~ 0x0C, little endian
	sector_size = (sector_size<<8) + buf[11];
	if(sector_size!=512) {
		uart_printf("invalid sector_size!\n");
		while (1) {}
	}
	unsigned int cluster_size = *(unsigned char*)(buf+13); // 0x0D
	if(cluster_size!=1) {
		uart_printf("invalid cluster_size!");
		while (1) {}
	}
	unsigned int table_beg = *(unsigned short*)(buf+14); // 0x0E
	unsigned int table_num = buf[16]; // 0x10 
	//unsigned int sector_num=*(unsigned short*)(buf+19);//need aligned
	unsigned int sector_num = buf[20]; // 0x13 ~ 0x14
